(function($) {
  ...
  const node = document.getElementById("intro-section");

  const cycleImages = (backgroundImgs, container, step) => {
    backgroundImgs.forEach((image, index) =>
      setTimeout(() => {
        container.style.backgroundImage = `url(${image})`;
      }, step * (index + 1))
    );
    setTimeout(
      () => cycleImages(backgroundImgs, container, step),
      step * backgroundImgs.length
    );
  };

  cycleImages(backgroundImgs, node, 3000);
